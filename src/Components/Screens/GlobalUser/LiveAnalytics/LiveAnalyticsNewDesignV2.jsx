  ...
  const LiveAnalyticsScreen = ({FactoryID,GlobalSummaryFlag,GlobalHeading,GlobalSubHeading}) => {
    ...
    async function fetchAnalyticsBars() {
      ...
      try {
        ...
        if (res.status === 200) {
          // Get the compliance and alert percentages from the response
          const compliancePercentage = Math.round(res.data.average_compliance_percentage);
          const alertPercentage = Math.round(res.data.average_alert_percentage);
  
          // Store compliance summary in state
          setComplianceSummaryData(res.data.compliance_summary);
  
          // Update the donut chart data
          setRecentOrderChart(prev => ({
            ...prev,
            series: [compliancePercentage, alertPercentage],
          }));
  
          // Rest of your existing code for progressData
          const progressData = res.data.compliance_summary
            ? res.data.compliance_summary.map(item => ({
                name: item.module,
                barValue: item.compliance_percentage,
                trend: item.compliance_percentage >= 80 ? 'uptrend' : 'downtrend',
                total_records: item.total_records,
                compliance_count: item.compliance_count,
                alert_count: item.alert_count,
                progressId: `progress_${item.module.toLowerCase().replace(/ /g, '_')}`,
                alerts: item.alert_count,
                percentage: `${item.compliance_percentage}%`,
                barColor:
                  item.compliance_percentage >= 80
                    ? 'success'
                    : item.compliance_percentage >= 50
                    ? 'warning'
                    : 'danger',
              }))
            : res.data.progressData;
  
          setOverAllComplaince(compliancePercentage);
          setProgressData(progressData);
          setProgressContext(progressData);
        }
    ...
    async function modelAccuracy() {
      ...
      try {
        ...
        if (res?.statusText?.toLocaleLowerCase() == "ok") {
          ...
          setAccuracyPercent(res.data.average_accuracy);
            setAiAccuracyContext(res.data.average_accuracy);
        }
        setAccuracyLoader(false);
      } catch (err) {
        console.log("Ai accuracy error", err);
    ...
    const handleWeekChange = (e) => {
      ...
      setFilters((prev) => {
        const update = {
          ...prev,
          month: "",
          weekly: !value ? getCurrentWeek() : value,
        };
      ...
      <Fragment>
        ...
        <Container fluid={true}>
          ...
          <Row>
            ...
            <>
              <Col xl='3'>  
              <NewCameraCount loader={connectivityFactoryLoader}/> 
              </Col>
              <Col xl='3'>  
              ...
              <Col xl='6'> 
                    ...
                    module={ highseverityCardData?.max_alerts ?
                      highseverityCardData?.max_alerts?.object === "forklift_person_in_same_aisle" ? "MMHE"
                      : highseverityCardData?.max_alerts?.object === "emergency_exit_blockage" ? "E.Exit"
                      : highseverityCardData?.max_alerts?.object === "Emergency Exit"? "E.Exit"
                      : highseverityCardData.max_alerts.object : null
                    }
                  ...
                  loadingForDonut ? <span className="w-100 h-100 d-flex justify-content-center align-items-center position-absolute"><Loader3 /></span>
                    ...
                    <> 
                      <CardBody className="position-relative p-0">
                        ...
                        <NewDonutChart seriess={recentOrderChart.series}  complianceSummary={complianceSummaryData} height={240} />
                        </Col>
                        <Col md='7' >
                          <NewBarChart  
                  ...
                  loadingForDonut ? <span className="w-100 h-100 d-flex justify-content-center align-items-center position-absolute"><Loader3 /></span>
                    ...
                    <> 
                      <CardBody className="pe-0">
                        <Row>
                        <Col md='5' sm='6' className="d-flex justify-content-center align-items-center  pe-0" >
                        <NewDonutChart seriess={recentOrderChart.series} height={240} />
                        </Col>
                        <Col md='7' sm='6' >
                          <NewBarChart  
                           progressData={progressData}
