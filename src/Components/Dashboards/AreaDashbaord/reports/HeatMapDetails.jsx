...
import HeatMapDetailsFilters from "./HeatMapDetailsFilters";
import liveAlertContext from '../../../../_helper/formData/LiveAlert/LiveAlert';



const HeatMapDetails = ({ togglee, settogglee, acceptedArray, setAcceptedArray, rejectedArray, setRejectedArray, runApi, setRunApi, setQaFilters }) => {
  const todayy = new Date().toISOString().split("T")[0];
  let subanalysis = undefined;
  const xxvalue = JSON.parse(localStorage.getItem('xAxisValue'))
  const processedXValue = xxvalue ? xxvalue.substring(0, 5) : '';
const xvalue = processedXValue.length >= 4 && processedXValue.charAt(3) === '0'
  ? processedXValue.substring(0, 3) + processedXValue.substring(4)
  : processedXValue;
  console.log('xvalue from local', xvalue)
  const yvalue = JSON.parse(localStorage.getItem('yAxisValue'))
  const subValue = JSON.parse(localStorage.getItem('subValue'))
  const heatmapfilters = JSON.parse(localStorage.getItem('heatmapfilters'))
  console.log('heatmap filters', heatmapfilters)
  ...
  const { setLiveAlertData, settotalLiveAlerts, setFiltersContext,setmodelandreports } = useContext(liveAlertContext);
  ...
  const [selectedOption, setSelectedOption] = useState(heatmapfilters.weekly!=='' ? 'Week' : 'Month');
  const [dateShow, setDateShow] = useState(false);
  const [monthShow, setMonthShow] = useState(heatmapfilters.month!=='' ? true : false);
  const [weeklyShow, setWeeklyShow] = useState(heatmapfilters.weekly!=='' ? true : false);
  ...
  const [newFilters, setNewFilters] = useState({
    ...
    module: yvalue,
    severity: "",
    shift: heatmapfilters.shift!=='' ? [heatmapfilters.shift] : [],
    date: "",
    week: heatmapfilters.weekly,
    month: heatmapfilters.month,
    starting: "",
    ending: "",
    area: xvalue,
    subarea: subValue,
