...
const Index = ({ togglee, settogglee, rejectedArray, setRejectedArray, runApi, setRunApi, setQaFilters }) => {
  ...
  const [verifiedmod, setverifiedmod] = useState({})

  async function fetchLive(payload, payload1) {

    try {
      setLoader(true);
      console.log('payload launchinggg', payload.filters)
      ...
      if (res) {
        ...
        const update = []
        res?.data?.data?.alerts.map((a)=>{
          if(a.helmet_accuracy==true || a.boots_accuracy==true || a.glasses_accuracy==true || a.mask_accuracy==true || a.gloves_accuracy==true){
            update.push(a.operation_safety_id)
          }
        })
        setAcceptedArray(update)

        setRejectedArray((prev)=>{
          const update = [...prev]
          res?.data?.data?.alerts.map((a)=>{
            if(a.helmet_accuracy==false || a.boots_accuracy==false || a.glasses_accuracy==false || a.mask_accuracy==false || a.gloves_accuracy==false){
              update.push(a.operation_safety_id)
            }
          })
          return update
...
res?.data?.data?.alerts.forEach((a) => {
  Object.entries(a).forEach(([key, value]) => {
    if (value==true && key !== "operation_safety_id") { 
      if (!verifiedData[a.operation_safety_id]) {
        verifiedData[a.operation_safety_id] = {}; // Initialize object for each ID
      }
      verifiedData[a.operation_safety_id][key] = value; // Store true values
    }
  });

  if (Object.keys(verifiedData[a.operation_safety_id] || {}).length > 0) {
    updated.push(a.operation_safety_id);
  }
});

setverifiedmod(verifiedData);
        ...
        if (filters.approval === 'Verified') {
          ...
          updatedState = {
            ...
            verified: res.data.data.verified_count,
        ...
        settotalLiveAlerts((prev)=>({
          ...prev,
          verified: res.data.data.accepted_records + res.data.data.rejected_records,
          accepted: res.data.data.accepted_records,
          rejected: res.data.data.rejected_records,
          pending: res.data.data.total_records - (res.data.data.accepted_records + res.data.data.rejected_records)
        }))
          ...
          loader ? (
            ...
            : (
              <>
                    ...
                    <div className={`${role !== 'qa' && 'mb-2'}`}>
                      <Row>
                        <Col md='7' className="">
                          ...
                          {newFilters?.area && role !== 'area' && <><span>Area: </span> <span className="f-light">{newFilters?.area}</span></>}
                          {newFilters?.subarea && <> <span>Sub Area: </span> <span className="f-light">{newFilters?.subarea}</span></>}
                          {newFilters?.module && <> <span>Module: </span> <span className="f-light">{newFilters?.module}</span></>}                        
                          </div>
                          <div className="d-flex gap-2 flex-wrap">

                          {newFilters?.severity && <><span>Severity: </span> <span className="f-light">{newFilters?.severity}</span></>}
                          {newFilters?.shift.length > 0 && <> <span>Shifts: </span> <span className="f-light">{newFilters?.shift.join(" | ")}</span></>}
                        ...
                        <Col md='5' className="d-flex  justify-content-end justify-content-xl-end justify-content-lg-end justify-content-md-end justify-content-sm-end ">
                          <div type='button' className={`d-flex justify-content-center filter-btnn  ${showFilters && 'border_R'}`}
                            ...
                            style={{ backgroundColor: '#635470', border: '1px solid #e0e0e0' }}
                          >
                            <p
                              className="m-0"
                            ...
                            <span className="d-flex"><RiMenu5Line color="#FFFFFF" size={16} className=" ms-2 " /></span>
                            {/* <span className="d-flex"><Filter color="#000" size={16} className="ms-2 " /></span> */}
                          </div>
                        </Col>
                      </Row>
                ...
                <LiveAlertsCards verifiedmod={verifiedmod} setverifiedmod={setverifiedmod}  
                               setFiltereddData={setFiltereddData} loader={loader} togglee={togglee} settogglee={settogglee} summary={summary} setsummary={setsummary} setAcceptedArray={setAcceptedArray} setRejectedArray={setRejectedArray} accept={accept} setAccept={setAccept} reject={reject} setReject={setReject} runApi={runApi} setRunApi={setRunApi} acceptedArray={acceptedArray} rejectedArray={rejectedArray} setLoader={setLoader} role={role} key={pageNo} setPageNo={setPageNo} pageNo={pageNo} total_pages={totalPages} imageData={imageData} showModal={showModal} setShowModal={setShowModal} filtereddData={filtereddData} handleCardClick={handleCardClick} />
