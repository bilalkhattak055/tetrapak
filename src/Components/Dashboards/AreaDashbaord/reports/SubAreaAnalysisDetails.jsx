...
const SubAreaAnalysisDetails = ({ togglee, settogglee, acceptedArray, setAcceptedArray, rejectedArray, setRejectedArray, runApi, setRunApi, setQaFilters }) => {
  const todayy = new Date().toISOString().split("T")[0];
  let subanalysis = undefined;
  subanalysis = JSON.parse(localStorage.getItem('subAreaPayload'))
  let subareaanalysisfilters = JSON.parse(localStorage.getItem('aifilterss'))
  ...
  const [newFilters, setNewFilters] = useState({
    ...
    module: "",
    severity: "",
    shift: subareaanalysisfilters.shifts!=='' ? [subareaanalysisfilters.shifts] : [],
    date: subareaanalysisfilters.date,
    week: subareaanalysisfilters.week,
    month: subareaanalysisfilters.month,
    starting: subareaanalysisfilters.starting,
    ending: subareaanalysisfilters.ending,
    area: subanalysis?.areaName,
    subarea: subanalysis?.subareaName,
  });
            ...
            subanalysis!==undefined &&(
                <div style={{paddingBlock:'16px'}} className='d-flex align-items-center gap-2'>
                    <SlArrowLeft onClick={()=> {
                     ...
                     navigate(`${process.env.PUBLIC_URL}/sub-area-analysis/${JSON.parse(localStorage.getItem('role'))}`)
...
export default SubAreaAnalysisDetails;
